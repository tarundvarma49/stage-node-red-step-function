  ...
  return when.promise(function (resolve, reject) {
    ...
    processTaskNodes(nodeDetail, previousNode, states,lambdaMapping).then((states) => {
      ...
      if (nodeDetail[0].wires.length === 0) {
        console.log('nodeDetail[0].wires.length---',nodeDetail[0].wires.length)
...
var processTaskNodes = function (nodeDetail, previousNode, states,lambdaMapping) {
  ...
  console.log('nodeDetail[0].type ---' + nodeDetail[0].type)
  console.log('lambdaMapping[nodeDetail[0].type] ---',lambdaMapping[nodeDetail[0].type])
  return when.promise(function (resolve, reject) {
    
    if (nodeDetail[0] && nodeDetail[0].type !== 'switch') {
      
      if(nodeDetail[0].type === 'createCognitoUser'){
        lambdaMapping[nodeDetail[0].type] = 'arn:aws:lambda:us-east-1:133013689155:function:wdev-nodered-cognitouser'
      }
      ...
      states['States'][nodeDetail[0].id.replace('.', '_')]['TimeoutSeconds'] = 300
       ...
       if(nodeDetail[0].type === createCognitoUser){
         console.log('=======================================',lambdaMapping[nodeDetail[0].type])
        states['States'][nodeDetail[0].id.replace('.', '_')]['Resource'] = 'arn:aws:lambda:us-east-1:133013689155:function:wdev-nodered-cognitouser'
       }else {
